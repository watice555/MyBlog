...
`npm run dev` 和 `npm run edit:local` 默认都使用 `http://localhost:3000`，避免因主机名或端口不同产生多套浏览器草稿与本机文章。如果 `3000` 已被其他程序占用，一键编辑脚本会自动向上查找可用端口（最多 100 个端口），并打印实际地址；也可以用 `PORT` 环境变量指定固定端口。
...
- 内容输入后会自动保存在当前浏览器；
- 顶部导航的「同步文章」等价于运行 `npm run content:generate`，会清除浏览器本地文章并让页面列表严格同步 `content/posts/`；
