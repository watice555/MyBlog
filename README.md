...
- 「正式保存并发布」或「正式保存修改」会把 Markdown 写入 `content/posts/`，执行 GitHub Pages 静态构建，然后定向提交当前文章、生成的文章索引和正文引用的本地图片，并推送到 `origin/main`；
- 本地页面会自动重新读取 `content/posts/`，所以从网页保存和直接修改 Markdown 文件效果相同；
- 尚未存入草稿箱的输入只存在当前编辑表单，刷新或关闭页面会丢失；
...
编辑器只允许从本机回环地址访问文件写入、Git 发布和 AI 接口，不在浏览器中保存 GitHub 密钥；推送使用本机已有的 Git 凭据。本机模型设置保存在不会提交的 `.local/llm-config.json` 中。
